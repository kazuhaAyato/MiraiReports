...
public class      bind extends Command {
    ...
    public void execute(CommandSender commandSender, String[] strings) {
        if (strings.length < 1) {
            commandSender.sendMessage(new TextComponent("§c用法错误. 正确用法: /qq <验证码>"));
            return;
        }
        if (qqbind.containsKey(Long.parseLong(strings[0]))) {
            if ((System.currentTimeMillis() - qqexpire.get(Long.parseLong(strings[0]))) > 600000) {
                ...
                commandSender.sendMessage(new TextComponent("§c验证码已过期！请尝试重新开始验证过程") );
                return;
            } else {
                try {
                    ...
                    if(set.next()){
                        commandSender.sendMessage(new TextComponent("§c你已经绑定过了！"));
                        set.close();
                        connection.close();
                        return;
                    }
                    ...
                    commandSender.sendMessage(new TextComponent("§a验证成功！你可以使用.unban了!"));
                } catch (SQLException throwables) {
                    throwables.printStackTrace();
                }
            }
        } else {
            commandSender.sendMessage(new TextComponent("§c错误：请先加入QQ群："+ config.getLong("player-group-num")+"输入.bind以开始验证！"));
            return;
        }
    }
}
